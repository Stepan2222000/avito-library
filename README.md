...
для Авито. Большинство готовых блоков можно найти в `avito_library/reuse_utils`.
Везде придерживаемся принципа KISS — решения должны оставаться максимально
...
- Если свободных прокси не осталось, очередь ставим на паузу и ждём, пока
  `ProxyPool` не сигнализирует о появлении новых адресов.
...
- Обработчики состояний не должны менять очередь напрямую — только через
  `TaskQueue.retry()` / `TaskQueue.mark_done()`, чтобы сохранить целостность.

## 9. Утилиты `reuse_utils`

| Утилита | Назначение | Когда применять |
|---------|------------|----------------|
| `TaskQueue` | Очередь задач с учётом повторных попыток и возможностью паузы | Всегда, когда нужно распределять работу между несколькими асинхронными воркерами |
| `ProcessingTask` | Контейнер с ключом задачи, полезной нагрузкой и метаданными | Используйте при работе с `TaskQueue`, чтобы хранить состояние задачи |
| `TaskState` | Перечисление внутренних состояний (`pending`, `in_progress`, `returned`) | Удобно для логирования и тестов |
| `ProxyPool` | Кольцевая выдача прокси, фиксация блокировок в файле, ожидание доступных адресов | Когда проект работает через список прокси, особенно с ограничениями по блокировкам |
| `ProxyEndpoint` | Описание одного прокси (`server`, `auth`, счётчики ошибок) | Полезно, если нужно анализировать или сериализовать конкретный адрес |

Для обработки капчи придерживайтесь подхода из раздела 4: наследуйте свой воркер
от базового класса проекта, храните в нём текущую `page` и `proxy`, вызываетe
`resolve_captcha_flow`, а решение о блокировке принимайте исходя из состояния
детектора. Общая логика описана выше, отдельный миксин не требуется.

СТОИТ ПРОСМОТРЕТЬ ПАПКУ avito_library/reuse_utils ДЛЯ ПОНИМАНИЯ ТОГО, как устроено то что уже настроено и желатлеьно переиспользвоать

Соблюдение этих правил позволяет писать совместимые приложения и свободно
комбинировать утилиты из `reuse_utils` без неприятных сюрпризов.
